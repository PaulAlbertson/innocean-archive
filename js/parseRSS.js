...
function parseRSSWP(){
	...
	function buildMenu(CompletedArray, menu) {
		...
		for (var i = 0; i < rssMasterArray.length; i++) {
			...
			var n=CompletedArray[i].date.toUpperCase().split(" ");
			if( n[0].indexOf('.') != -1 ) n[0] =  n[0].substring(0, n[0].length - 1);
			
			var dateHTML = '<h1>' + n[0] + " " + n[2] + '</h1>';
			var titleHTML = '<h2>' +  CompletedArray[i].title + '</h2>';
			
			 var articleSourceHTML = '<h3>' +  CompletedArray[i].articleSource + '</h3>';
		
			 $(menu).append("<div class='newsitems' data=" +  i +  ">" + dateHTML + titleHTML + articleSourceHTML + "</div>"
		...
		var dateHTML = '<h1>' + CompletedArray[currentSelection].date + '</h1>';
		$('#pressContent').append($(dateHTML));
		...
		var contentHTML = '<p>' +  CompletedArray[currentSelection].content + '</p>';
