...
$(document).ready(function () {
    ...
    function g(a, c, d) {
        ...
        for (var b = $(a).children().length, b = Math.round(b / 12), f = $(a).attr("category"), q = ["left", "center", "right"], e = 1; e <= b; e++) $(c).append("<li></li>");
        $(a).find("item").each(function (a) {
            ...
            var b = $(this).find("title").text(),
              ...
              a < 12 ? $(c).find("li").eq(0).append(b) : a < 24 ? $(c).find("li").eq(1).append(b) : a < 46 ? $(c).find("li").eq(2).append(b) : a < 48 ? $(c).find("li").eq(3).append(b) : a < 60 ? $(c).find("li").eq(4).append(b) : a < 72 ? $(c).find("li").eq(5).append(b) : a < 84 ? $(c).find("li").eq(6).append(b) : a < 96 ? $(c).find("li").eq(7).append(b) : a < 108 ? $(c).find("li").eq(8).append(b) : a < 120 && $(c).find("li").eq(9).append(b)
    ...
    function l(a, c) {
        ...
        if(aId[1] ==  "#experienceriptide" ) {
        	...
        	$("#project-drawer").css({"height" : "650px" , "overflow" : "hidden" , "background" : "#000 url('../images/lines-dark.png') repeat"});
        }
	...
	$("#twitter_footer").click(function() { 
		window.open("https://twitter.com/innoceanusa", "", "_blank");
	 });
	
	$("#fb_footer").click(function() { 	
		window.open("https://www.facebook.com/InnoceanUSA", "", "_blank");
	 });
	...
	$("#fb_footer").hover(function() { 
	 ...
	 $("#cam_footer").hover(function() { 
		$(this).css("background-position", "-96px -41px"); 
		$(this).css( 'cursor', 'pointer' );
		$("#nav_footer").css( {
				'width': '377',
				'height': '269',
            ...
            $("#marquee-carousel .marquee-link a.large").click(function (a) {
				...
				if( $(this).attr("id") == "wrapparty" ) {
					...
					$("#about, #about-nav, #about .nav-item, #about .nav-spacer").removeClass("orange").removeClass("dark").addClass("blue");
   					$("#riptide").hide();
   					$("#about").css({"padding" : "0 0 0px"});
   					$("#container_wrapparty").css({"background" : "#000 url('../images/about-us-shag.gif') center center no-repeat"});
   					
   					$("#iframe-wrapparty").attr("src", "shag/index.html" );
    ...
    $("#about-toggle").click(function () {
       	...
       	$("#about, #about-nav, #about .nav-item, #about .nav-spacer").removeClass("dark").removeClass("blue").addClass("orange");
     	$("#about").css({"padding" : "0 0 40px"});
	...
	$('a.lightbox').lightBox(); 
	 ...
	 $("#wrapparty-toggle").click(function () {
		...
		//console.log("parseMyRSS", parseMyRSS);
